...
document.addEventListener("DOMContentLoaded", function(event) {
    let superHeroes = JSON.parse(jsonFile);

    let superHeroesCards = "";
    let superHeroesCardsBack = "";
    for (let superHero of superHeroes) {
        superHeroesCards += `<div class="sh-card">
        <h2>${superHero.name}</h2>
        <p>${superHero.universe}</p>
        <p>Альтер-эго: ${superHero.alterego}</p>
        <p>${superHero.occupation}</p>
        <p>Друзья: ${superHero.friends}</p>
        <p>Суперсила: ${superHero.superpowers}</p>
        <img src=${superHero.url} width=250px/>
        </div>`;
        superHeroesCardsBack += `<p class="sh-card__back>${superHero.info}</p>`;
    }
    document.querySelector('.card__front').innerHTML = superHeroesCards;
    document.querySelector('#info').innerHTML = superHeroesCardsBack;
});
